...
export const FloatingWhatsApp: React.FC<FloatingWhatsAppProps> = ({ agencyConfig }) => {
  ...
  } else if (!cleanNumber.startsWith('91') && cleanNumber.length > 0) {
    cleanNumber = '91' + cleanNumber;
  }
  const targetNumber = cleanNumber || '919789504702';
  ...
  const handleOpenWhatsApp = (e: React.MouseEvent<HTMLAnchorElement>) => {
    // If inside restricted iframe where target="_blank" may be blocked, ensure navigation
    try {
